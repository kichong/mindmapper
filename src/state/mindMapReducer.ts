...
type MindMapAction =
  ...
  | { type: 'SET_SELECTED_SHAPES'; shapeIds: string[] }
  | { type: 'MOVE_SHAPES'; updates: { shapeId: string; x: number; y: number }[] }
  | { type: 'COLOR_SHAPES'; shapeIds: string[]; color: string }
...
export const STORAGE_KEY = 'mindmapper:snapshot'

export function loadPersistedState(): MindMapState {
  ...
  try {
    ...
    return {
      ...
      selectedShapeId,
      selectedShapeIds: selectedShapeId ? [selectedShapeId] : [],
      history: {
        past: [],
        future: [],
      },
...
// Reducer updates are immutable, so history can share unchanged arrays and records.
function snapshotOf(state: MindMapState): MindMapSnapshot {
  return {
    nodes: state.nodes,
...
function reduceMindMap(state: MindMapState, action: MindMapAction): MindMapState {
  switch (action.type) {
    case 'ADD_NODES': {
      ...
      action.nodes.forEach((node) => {
        ...
        newNodes.push({ ...node, textSize: normalizeTextSize(node.textSize) })
      })
    ...
    case 'ADD_NODE': {
      const nextNodes = [...state.nodes, { ...action.node, textSize: normalizeTextSize(action.node.textSize) }]
      return commitState(state, {
        nodes: nextNodes,
    ...
    case 'UPDATE_NODE': {
      return updateNodes(state, [{ nodeId: action.nodeId, updates: action.updates }])
    }
    case 'UPDATE_NODES': {
    ...
    case 'ADD_ANNOTATION': {
      const nextAnnotations = [...state.annotations, { ...action.annotation, textSize: normalizeTextSize(action.annotation.textSize) }]
      return commitState(state, {
        annotations: nextAnnotations,
    ...
    case 'UPDATE_ANNOTATION': {
      const nextAnnotations = state.annotations.map((annotation) =>
        annotation.id === action.annotationId
          ? { ...annotation, ...action.updates, id: annotation.id, textSize: normalizeTextSize(action.updates.textSize ?? annotation.textSize) }
          : annotation,
      )
    ...
    case 'UNDO': {
      ...
      return {
        ...
        annotations: mergedSnapshot.annotations,
        shapes: mergedSnapshot.shapes,
        crossLinks: mergedSnapshot.crossLinks,
        selectedNodeIds,
        selectedAnnotationId,
        selectedShapeId,
        selectedShapeIds: state.selectedShapeIds,
...
// Keep the primary shape for the single-shape inspector; group selection is runtime-only.
export function mindMapReducer(state: MindMapState, action: MindMapAction): MindMapState {
  if (action.type === 'SET_SELECTED_SHAPES') {
    const existing = new Set(state.shapes.map((shape) => shape.id))
    const ids = [...new Set(action.shapeIds)].filter((id) => existing.has(id))
    return { ...state, selectedShapeIds: ids, selectedShapeId: ids[0] ?? null,
      selectedNodeIds: ids.length ? [] : state.selectedNodeIds,
      selectedAnnotationId: ids.length ? null : state.selectedAnnotationId }
  }
  if (action.type === 'MOVE_SHAPES') {
    const positions = new Map(action.updates.map((update) => [update.shapeId, update]))
    let changed = false
    const shapes = state.shapes.map((shape) => {
      const position = positions.get(shape.id)
      if (!position || !Number.isFinite(position.x) || !Number.isFinite(position.y) ||
          (position.x === shape.x && position.y === shape.y)) return shape
      changed = true
      return { ...shape, x: position.x, y: position.y }
    })
    return changed ? { ...state, shapes } : state
  }
  if (action.type === 'COLOR_SHAPES' || action.type === 'DELETE_SHAPES') {
    const ids = new Set(action.shapeIds)
    const shapes = action.type === 'DELETE_SHAPES'
      ? state.shapes.filter((shape) => !ids.has(shape.id))
      : state.shapes.map((shape) => ids.has(shape.id) && shape.color !== action.color
        ? { ...shape, color: action.color } : shape)
    if (shapes.length === state.shapes.length && shapes.every((shape, i) => shape === state.shapes[i])) return state
    const remaining = state.selectedShapeIds.filter((id) => shapes.some((shape) => shape.id === id))
    return { ...commitState(state, { shapes, selectedShapeId: remaining[0] ?? null }), selectedShapeIds: remaining }
  }
  const next = reduceMindMap(state, action)
  if (next === state) return state
  const existingShapeIds = new Set(next.shapes.map((shape) => shape.id))
  const ids = action.type === 'SELECT_SHAPE' || action.type === 'ADD_SHAPE'
    ? (next.selectedShapeId ? [next.selectedShapeId] : [])
    : !next.selectedShapeId ? []
    : state.selectedShapeIds.filter((id) => existingShapeIds.has(id))
  return { ...next, selectedShapeIds: ids, selectedShapeId: ids[0] ?? null,
    selectedNodeIds: ids.length ? [] : next.selectedNodeIds,
    selectedAnnotationId: ids.length ? null : next.selectedAnnotationId }
}
